...
import numpy as np
from dpt_models.embedder import get_embedder
...
class RenderingNetwork(nn.Module):
    ...
    def forward(self, points, normals, view_dirs, feature_vectors):
        ...
        if self.squeeze_out:
            ...
            # print('[Warning] relu_n_sig. For Depth_extract_net only')
            x = self.relu(x)
            # x = torch.sigmoid(x)
        return x


class DepthFeatNet(nn.Module):
    def __init__(self,
                 d_in,
                 d_out,
                 d_hidden,
                 ...
                 weight_norm=True,
                 inside_outside=False):
        super(DepthFeatNet, self).__init__()
