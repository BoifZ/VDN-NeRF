...
class NeuSRenderer:
    ...
    def render_core_outside(self, rays_o, rays_d, z_vals, sample_dist, nerf, background_rgb=None):
        ...
        # add depth_feats
        if self.depth_network is not None:
    ...
    def render_core(self,
        ...
        # add depth_feats
        if depth_network is not None:
            sampled_feat = depth_network(pts, gradients, dirs, feature_vector)
            if depth_before_color:
                feature_vector = torch.cat([feature_vector, sampled_feat], dim=-1)
            sampled_feat = sampled_feat.reshape(batch_size, n_samples, -1)
    ...
    def render(self, rays_o, rays_d, near, far, perturb_overwrite=-1, background_rgb=None, cos_anneal_ratio=0.0, depth_before_color=False):
        ...
        if self.n_outside > 0:
            ...
            background_zvals = ret_outside['z_vals']
        ...
        ret_fine = self.render_core(rays_o,
                                    ...
                                    self.color_network,
                                    self.depth_network,
                                    depth_before_color=depth_before_color,
        ...
        if background_zvals is not None:
            # z_vals = torch.cat([z_vals,background_zvals], dim=1)
            z_vals = background_zvals
